...
mod frontend_tests {
    ...
    fn test_frontend_initialization() {
        ...
        initial_frontend.backend.set_cell(2, 3, Value::Number(42.0)).unwrap();
        initial_frontend.backend.set_cell(4, 5, Value::Text("Test".to_string())).unwrap();
        initial_frontend.backend.serial(path_str).expect("Failed to serialize");
    ...
    fn test_print_grid_enabled() {
        ...
        frontend.backend.set_cell(1, 1, Value::Number(100.0)).unwrap();
        frontend.backend.set_cell(2, 2, Value::Text("Test".to_string())).unwrap();
